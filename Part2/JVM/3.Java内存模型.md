...
防止**指令重排**，否则引发半初始化问题
由于发生了指令重排，假如第一个线程给单例对象t分配内存后，先把内存引用赋值给声明的变量t，再执行对象初始化（正常情况是先初始化再把引用给t），这样对象初始化害没完成，第二个线程通过双重校验判断t已经不为null了，就会直接使用单例对象t，但他拿的是半初始化状态的t，如果直接使用t未初始化的成员变量，就会出现问题
...
>
> -XX:+UseCompressedOops（oops = ordinary object pointers）：普通对象指针，是引用类型的成员变量，不是classpointers，**不可混淆**（64位前提下，不压缩8，压缩了4）
