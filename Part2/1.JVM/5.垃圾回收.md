...
3. 怎么回收？
   4. <a href="#GC算法" style="text-decoration: none;">GC算法</a>
   5. <a href="#垃圾回收器" style="text-decoration: none;">垃圾回收器</a>
...
一句话总结：对象被**方法局部变量**、**类静态变量**引用，就不会被回收
...
> ```
>
> 不能分配则判断	是否大于虚拟机参数指定大小，超过了直接分配Old区
>
...
- 动态年龄判断进入 
  ...
  年龄1+年龄2+....+年龄n的所有对象大小超过Survivor区的50%，就会把年龄n以上的对象放入老年代

- 分配担保进入

  每次Minor GC之前，JVM会检查Old去可用空间，是否大于Young区所有对象总大小，避免出现Minor GC之后所有对象都没被回收的极端情况	

  如果Old可用空间大于之前每次Minor GC后进入Old对象的平均大小
