...
```
通过将max.poll.interval.ms设置成实际的逻辑处理时间，再结合较低的session.timeout.ms参数值，consumer group 既实现了快速的 consumer 崩溃检测，也保证了复杂的事件处理逻辑不会造成不必要的 rebalance（因为如果consumer消费过慢，coordinator会将其移出group，它负责的partition会分配给其他consumer，引发不必要的rebalance
```
